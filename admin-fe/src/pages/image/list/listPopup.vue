      ...
      <standard-table :columns="columns" :dataSource="dataSource" @clear="onClear" @change="onChange"
        ...
        <div slot="action" slot-scope="{ text, record }" style="display: flex; flex-direction: column;">
          <a style="margin-right: 8px">
            <a-icon type="edit" />
            <router-link :to="`/popup/new?id=${record.id}`">Edit
            </router-link>
          </a>
...
export default {
  ...
  methods: {
    ...
    getData() {
      request(process.env.VUE_APP_API_BASE_URL + "/popup", "get", {
    ...
    deleteRecord(id) {
      ...
      if (!confirm) return;
      request(process.env.VUE_APP_API_BASE_URL + "/popup/" + id, METHOD.DELETE).then(() => {
        this.getData();
