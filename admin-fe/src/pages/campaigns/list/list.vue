                ...
                <standard-table :columns="columns" :dataSource="dataSource"
                    ...
                    <div slot="action" slot-scope="{  record }">
                        <a style="margin-right: 8px">
                            <a-icon type="edit" />
                            <router-link :to="`/campaigns/new?id=${record.id}`">Edit
                            </router-link>
                        </a>
...
export default {
    ...
    methods: {
        ...
        getData() {
            request(process.env.VUE_APP_API_BASE_URL + "/campain", "get", {
                ...
                this.dataSource = data.map((_data) => {
                    const isHot = _data.is_hot === 1;
                    const isBeginner = _data.is_beginner === 1;
                    let type = "";
                    if (isHot && isBeginner) {
                        type = "Hot, Beginner";
                    } else if (isHot) {
        ...
        deleteRecord(id) {
            ...
            if (!confirmed) return;
            request(process.env.VUE_APP_API_BASE_URL + "/campain/" + id, METHOD.DELETE).then(() => {
                this.getData();
            });
        },
        toggleAdvanced() {
