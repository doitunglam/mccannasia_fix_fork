...
<div class="dropdown-menu dropdown-menu-end view-p">
    <a class="dropdown-item" href="{{route('user.update', Auth::user()->id)}}"><i class="fas fa-pencil-alt font-size-14 align-middle me-1"></i> <span key="t-logout">{{ __trans($language, 'All.personal_information', 'Personal information') }}</span></a>
    <a class="dropdown-item text-danger" href="{{route('auth.signout')}}"><i class="bx bx-power-off font-size-16 align-middle me-1 text-danger"></i> <span key="t-logout">Logout</span></a>
</div>